...
std::string String::emptyString()
{
    return std::string("");
}

std::string String::whiteSpace() 
{
    return std::string(" \t");
}
 ...
 * Allows cout << String
 */
std::ostream& profiler::operator<<(std::ostream& os, String str)
{
  os << str.str_;
  return os;
}
					      ...
					      bool matchEndOfString)
{
  ...
  for(unsigned iMatch = iEl; iMatch < str_.size(); iMatch++) {
      if(matches(str_[iMatch], separators) || matches(str_[iMatch], whiteSpace()))
      iEl++;
    else
      break;
  }
...
void String::advanceToNextNonWhitespaceChar()
{
  for(; iStart_ < str_.size(); iStart_++) {
      if(!matches(str_[iStart_], whiteSpace()))
      break;
  }
}
